...
class RadarrAPIService: ObservableObject { // ADDED ObservableObject conformance
    ...
    init(settings: RadarrSettings) {
        ...
        self.baseURL = url // Initialize 'baseURL'
        ...
        self.session = URLSession(configuration: config) // Initialize 'session' last
    }
    ...
    private func performRequest<T: Decodable>(endpoint: String, method: String = "GET", body: Data? = nil) async throws -> T {
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            if let errorDetailArray = try? jsonDecoder.decode([RadarrErrorDetail].self, from: data), let firstError = errorDetailArray.first {
                 throw RadarrError.apiError(message: firstError.resolvedErrorMessage ?? "Radarr API Error \(httpResponse.statusCode)", statusCode: httpResponse.statusCode)
            } else if let singleErrorDetail = try? jsonDecoder.decode(RadarrErrorDetail.self, from: data) {
                 throw RadarrError.apiError(message: singleErrorDetail.resolvedErrorMessage ?? "Radarr API Error \(httpResponse.statusCode)", statusCode: httpResponse.statusCode)
            } else if let errorString = String(data: data, encoding: .utf8), !errorString.isEmpty {
                throw RadarrError.apiError(message: "Radarr API Error \(httpResponse.statusCode): \(errorString)", statusCode: httpResponse.statusCode)
            }
            throw RadarrError.apiError(message: "Radarr API Error \(httpResponse.statusCode)", statusCode: httpResponse.statusCode)
        }
    ...
    func addMovie(_ movie: RadarrMovie, addOptions: RadarrAddOptions, rootFolderPath: String, qualityProfileId: Int) async throws -> RadarrMovie {
    ...
    func searchForMovie(_ movieId: Int) async throws -> RadarrCommandResponse {
     ...
     func deleteMovie(_ movieId: Int, deleteFiles: Bool = false, addImportExclusion: Bool = false) async throws {
         ...
         guard let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) else {
             throw RadarrError.apiError(message: "Failed to delete movie. Status: \((response as? HTTPURLResponse)?.statusCode ?? 0)", statusCode: (response as? HTTPURLResponse)?.statusCode ?? 0)
         }
     }
